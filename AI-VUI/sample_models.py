...
def deep_rnn_model(input_dim, units, recur_layers, output_dim=29):
    ...
    simp_rnns = [None]*recur_layers
    bn_rnns = [None]*(recur_layers+1)
    bn_rnns[0] = input_data
    for i in range(recur_layers):
        simp_rnns[i] = GRU(units, return_sequences=True, implementation=2, name='rnn'+str(i))(bn_rnns[i])
        bn_rnns[i+1] = BatchNormalization()(simp_rnns[i])
    # TODO: Add a TimeDistributed(Dense(output_dim)) layer
    time_dense = TimeDistributed(Dense(output_dim))(bn_rnns[recur_layers])
    # Add softmax activation layer
    y_pred = Activation('softmax', name='softmax')(time_dense)
    # Specify the model
    model = Model(inputs=input_data, outputs=y_pred)
