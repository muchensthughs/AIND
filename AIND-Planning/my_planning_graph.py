...
class PlanningGraph():
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        a1_c_a2 = [f in node_a1.action.effect_rem for f in node_a2.action.effect_add]
        a2_c_a1 = [f in node_a2.action.effect_rem for f in node_a1.action.effect_add]

        if any(a1_c_a2) or any(a2_c_a1):
            return True
        return False
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        a1_i_a2 = [f in node_a1.action.effect_rem for f in node_a2.action.precond_pos] + [f in node_a1.action.effect_add for f in node_a2.action.precond_neg]
        a2_i_a1 = [f in node_a2.action.effect_rem for f in node_a1.action.precond_pos] + [f in node_a2.action.effect_add for f in node_a1.action.precond_neg]
        if a1_i_a2 or a2_i_a1:
            return True
        return False
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        for p1 in node_s1.parents:
            for p2 in node_s2.parents:
                if p1 == p2 or not p1.is_mutex(p2):
                    return False
        return True
    ...
    def h_levelsum(self) -> int:
        ...
        for i, level in enumerate(self.s_levels):
            for node_s in level:
                if expr(node_s.symbol) in goals and node_s.is_pos:
                    goals.remove(expr(node_s.symbol))
                    level_sum += i
                    if len(goals) == 0:
                        return level_sum
        return int('inf')
