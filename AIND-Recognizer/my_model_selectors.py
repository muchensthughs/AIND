    ...
    def select(self):
        ...
        for n_components in range(self.min_n_components, self.max_n_components+1):
            try:
                ...
                p = n_components * (n_components-1) + (n_components-1) + 2 * model.n_features * n_components
                score = -2 * logL + p * logN
            ...
            try:
                ...
                for word in self.hwords.keys():
                    if not word == self.this_word:
                        ...
                        other_scores_sum += model.score(X, lengths)
                other_scores_ave = other_scores_sum / (len(self.hwords)-1)
                DIC = logL - other_scores_ave
                if DIC > highest_score:
                    best_model = model
                    highest_score = DIC
            except:
                continue
